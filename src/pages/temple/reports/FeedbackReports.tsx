...
const FeedbackReports = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Trends */}
      <Card><CardHeader className="pb-2"><CardTitle className="text-base">Rating & Response Rate Trend</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={300}><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="hsl(0,0%,90%)" /><XAxis dataKey="month" /><YAxis yAxisId="left" domain={[3, 5]} /><YAxis yAxisId="right" orientation="right" domain={[70, 100]} /><Tooltip /><Line yAxisId="left" type="monotone" dataKey="rating" stroke="hsl(45,90%,45%)" strokeWidth={2} dot={{ r: 4 }} name="Avg Rating" /><Line yAxisId="right" type="monotone" dataKey="responseRate" stroke="hsl(142,60%,40%)" strokeWidth={2} dot={{ r: 4 }} name="Response %" /></LineChart></ResponsiveContainer></CardContent></Card>

      <Card><CardHeader className="pb-2"><CardTitle className="text-base">Monthly Feedback Volume</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={250}><BarChart data={monthlyReport.map(r => ({ month: r.month.split(" ")[0], positive: r.positive, negative: r.negative })).reverse()}><CartesianGrid strokeDasharray="3 3" stroke="hsl(0,0%,90%)" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Bar dataKey="positive" fill="hsl(142,60%,40%)" name="Positive" radius={[4,4,0,0]} stackId="a" /><Bar dataKey="negative" fill="hsl(350,65%,50%)" name="Negative" radius={[4,4,0,0]} stackId="a" /></BarChart></ResponsiveContainer></CardContent></Card>

      {/* Sentiment */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card><CardHeader className="pb-2"><CardTitle className="text-base">Sentiment Distribution</CardTitle></CardHeader><CardContent><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={sentimentData} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, value }) => `${name}: ${value}%`}>{sentimentData.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-base">Sentiment Summary</CardTitle></CardHeader><CardContent className="space-y-4">{sentimentData.map(s => (<div key={s.name}><div className="flex items-center justify-between mb-1"><span className="text-sm font-medium">{s.name}</span><span className="text-sm font-bold">{s.value}%</span></div><Progress value={s.value} className="h-3" /></div>))}</CardContent></Card>
      </div>
    </div>
  );
};
